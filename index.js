  ...
  jsonData.forEach((flightData) =>
    flightData.depart.forEach((flight, count) => {
      ...
      for (let i = 0; i < legs.length; i++) {
        ...
        const arrivalZone = airports
          .findWhere({ iata: arriveAirport })
          .get("tz");
        ...
        const departingFrom =
          airports.findWhere({ iata: departAirport }).get("name") +
          ", " +
          departAirport +
          ", " +
          airports.findWhere({ iata: departAirport }).get("city") +
          ", " +
          airports.findWhere({ iata: departAirport }).get("country");
        const arrivingAt =
          airports.findWhere({ iata: arriveAirport }).get("name") +
          ", " +
          arriveAirport +
          ", " +
...
function parseDurationInt(duration) {
  ...
  if (minutes >= 60) {
    ...
    if (hours >= 24) {
      ...
      hoursString =
        hours - days * 24 !== 0
          ? hours - days * 24 + " Hour" + (hours - days * 24 > 1 ? "s " : " ")
          : "";

      durationString =
        days + " Day" + (days > 1 ? "s " : " ") + hoursString + minutesString;
    } else {
